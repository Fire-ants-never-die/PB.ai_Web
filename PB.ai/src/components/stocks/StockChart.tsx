...
export function StockChart({
  ...
  const chartData = useMemo(() => {
    // 添加安全检查
    const safeCurrentPrice = typeof currentPrice === 'number' && !isNaN(currentPrice) ? currentPrice : 100;
    const safeVolatility = typeof volatility === 'number' && !isNaN(volatility) ? volatility : 2;

    const prices = generatePriceHistory(selectedRange.days, safeCurrentPrice, safeVolatility);
    const data = [];
    ...
    for (let i = 0; i < prices.length; i++) {
      ...
      const price = typeof prices[i] === 'number' && !isNaN(prices[i]) ? prices[i] : safeCurrentPrice;
  ...
  // Calculate min and max for Y axis with some padding - 添加安全检查
  const validPrices = chartData.map(d => d.price).filter(price => typeof price === 'number' && !isNaN(price));
  const minPrice = validPrices.length > 0 ? Math.min(...validPrices) * 0.98 : 0;
  const maxPrice = validPrices.length > 0 ? Math.max(...validPrices) * 1.02 : 100;
  ...
  return (
    <Card className={cn("overflow-hidden h-full", className)}>
      <CardHeader className="flex-row items-center justify-between pb-4">
        <div>
          <CardTitle className="leading-none">{symbol || 'N/A'}</CardTitle>
